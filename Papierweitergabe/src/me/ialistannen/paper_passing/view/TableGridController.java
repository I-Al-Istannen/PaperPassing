...
public class TableGridController {
	...
	private void initialize() {	
		...
		tableView.heightProperty().addListener((observable, oldHeight, newHeight) -> {
			resizeComponents(
		...
		data.addListener((ListChangeListener<StudentsGridEntry[]>) change  -> {
			resizeComponents(
					tableView.getWidth() - tableView.getInsets().getLeft() - tableView.getInsets().getRight()
							- tableView.getPadding().getLeft() - tableView.getPadding().getRight(),
					tableView.getHeight() - tableView.getInsets().getTop() - tableView.getInsets().getBottom()
	...
	private void createColumns(int width) {
		...
		tableView.setOnKeyPressed(event -> {
			if(event.getCode() == KeyCode.ESCAPE) {
			...
			else if(event.getCode() == KeyCode.B && event.isShiftDown()) {
				...
				getTableStudentFromUser().ifPresent(student -> {
					replaceSelectedEntry(student);
				});
			}
